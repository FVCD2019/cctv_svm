...
class SVM:
	...
	self.final_pub = rospy.Publisher("svm0",Image,queue_size=1)

    def pose(self, center, box, heading): 
	global camera_cen_x, camera_cen_y 
        self.ego_x, self.ego_y = center
        self.theta = heading
        ...
        for i in range(4):
            dis[i] = math.sqrt( math.pow(camera_cen_x - box[i,0], 2) +  math.pow(camera_cen_y - box[i,1], 2) )
    ...
    def stitching(self, image0, image1): #pending

        self.queue.put(image0)
	...
	prev_image0 = self.queue.get() #image before 30frame
	prev_image1 = prev_image0.copy()
	# stitching

	self.rows, self.cols = image0.shape[:2]
	self.M1 = np.float32([[1,0,650],[0,1,600]]) # translation x+650, y+600
	self.img_trans1 = cv2.warpAffine(image0, self.M1, (2*self.cols, 2*self.rows))            
	self.img_trans3 = cv2.warpAffine(prev_image0, self.M1, (2*self.cols, 2*self.rows))
	...
	self.img_trans2 = cv2.warpAffine(image1, self.M2, (2*self.cols, 2*self.rows)) 
	self.img_trans4 = cv2.warpAffine(prev_image1, self.M2, (2*self.cols, 2*self.rows))
	...
	self.final_pub.publish(self.bridge.cv2_to_imgmsg(self.final,"bgr8"))
	t3 = time.time()
